...
const char* test_descriptions[] = {
    ...
    /* 17 */ "alloc & free integrity test",
    /* 18 */ "alloc, free, & resize integrity test",
    /* 19 */ "alloc & free data integrity test",
    /* 20 */ "alloc, free, & resize data integrity test",
    /* 21 */ "alloc, free, resize - a lot of times",
    /* 22 */ "EMPTY TEST",
    /* 23 */ "EMPTY TEST",
    /* 24 */ "EMPTY TEST",
    /* 25 */ "EMPTY TEST",
...
int test04() {
    ...
    for (int i = 0; i < 9; i++) {
        int *array = hl_alloc(heap, ALIGNMENT * 8);
        if ((int)array % ALIGNMENT != 0) {
            result = false;
        }
    }
...
int test05() {
    ...
    char heap[HEAP_SIZE];
    int *fake = heap - 1;
    if ((int)fake % ALIGNMENT == 0) {
        printf("Aligned!");
        result = false;
    }
    
    hl_init(fake, HEAP_SIZE);
    

    for (int i = 0; i < 9; i++) {
        int *array = hl_alloc(fake, ALIGNMENT * 8);
        if ((int)array % ALIGNMENT != 0) {
            result = false;
        }
    }
...
int test06() {
    ...
    hl_init(heap, HEAP_SIZE);
    int *array = hl_alloc(heap, 32);

    heap[8] = (int)9;
    //printf("heap 8 is %d! /n", heap[8]);

    int *fakeblock = 0;
    hl_release(heap, fakeblock);
