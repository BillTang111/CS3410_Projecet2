#include "cachecollusion.h"

/* TitForTat Bot. Core 3 watches the taunt array to try and catch core, and
 * the remaining three cores cooperate to fill our half of memory. Each time
 * a core is noticed in enable_proxy mode, we send over one of the cores to start filling opponent's
    ...
    while (1) {
      ...
      for (i = 0; i < TAUNT_SIZE; i++) {
        if (HOME_STATUS->taunt[i] >= 0) {
          report_to_twitter(HOME_STATUS->taunt[i]);
          num_spies_seen++;
