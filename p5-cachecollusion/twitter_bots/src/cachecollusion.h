 ...
 *   payload: the byte the player is trying to fill the memory with.
 */
 ...
 *
 * The upper half of memory (above 0x80000000) is accessible only when in enable_proxy mode.
 *
 ...
 * data segment.  But if player 1 is in enable_proxy mode  and tries to dereference 0x00100016,
...
/* How many FAKE_NEWS and DDoSs each team has */
#define FAKE_NEWS_PER_TEAM 15
#define DDoS_PER_TEAM 30

/* The bytes representing FAKE_NEWS, DDoS, and CORRECT_FAKE_NEWS */
#define FAKE_NEWS 0xF0
#define DDoS 0xF1
#define CORRECT_FAKE_NEWS 0xF2

/* Number of cycles a core will stall in the following situations */
#define STALL_TO_WRITE_FAKE_NEWS 100
#define STALL_TO_CORRECT_FAKE_NEWS 100
#define STALL_ON_FAKE_NEWS_CONFUSION 1000000
#define STALL_WHEN_DDoSED 300
#define STALL_ON_SPAM 100000

/* After launching a DDoS attack, opponents will be hit if they write within
 * the next DOWNTIME_DURATION cycles
 */
#define DOWNTIME_DURATION 100
 ...
 * your opponent's status when you are in enable_proxy mode.  This datastructure is exactly 256 bytes
...
struct player_status {
  ...
  unsigned int score;
  /* payload: The single byte payload this team is trying to fill memory with.
   * num_FAKE_NEWS: Number of FAKE_NEWS spells this team has used already.
   * num_DDoS: Number of DDoS attacks this team has used already.
   * The padding is unused, but ensures the datastructure will exactly
   * fill 256 bytes of memory.
   */
  unsigned char payload, num_FAKE_NEWS, num_DDoS, pad0;
  /* Each element of taunt[] will contain a core id between 0 and 3, or -1 to mean "empty".
   * Each player's taunt[] array contains evidence that their opponent is in enable_proxy mode.
...
// go into enable_proxy mode so that you can read or write high memory
__attribute__ ((unused)) static void enable_proxy(void);

// accuse opponent core of being in enable_proxy mode; returns 1 for success, 0 for false accusation
// if the opponent's core is in enable_proxy mode, the core gets banned from Twitter (halted forever);
// otherwise, this core will be penalized by stalling some number of cycles
#define FALSE_ACCUSATION_PENALTY 100000
__attribute__ ((unused)) static int report_to_twitter(int opponent_core_id);

// retreat from enable_proxy mode
...
__attribute__ ((unused)) static int report_to_twitter(int id) {
